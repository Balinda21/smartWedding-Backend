...
import { CreateDJDTO, UpdateDJDTO } from "../../types/dj/djTypes";

export class DJService {
  async createDJ(data: CreateDJDTO) {
    try {
      const dj = await prisma.dJ.create({ data });
      return {
        status: true,
  ...
  async updateDJ(id: number, data: UpdateDJDTO) {
    try {
      const existingDJ = await prisma.dJ.findUnique({ where: { id } });
